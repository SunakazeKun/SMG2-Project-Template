...
void MoguStone::attackSensor(HitSensor* pHit1, HitSensor* pHit2) {
    if (pHit1 == getSensor("Body") && MR::isSensorPlayer(pHit2)) {
        if (MR::sendMsgEnemyAttack(pHit2, pHit1)) {
...
u32 MoguStone::receiveMsgPlayerAttack(u32 msg, HitSensor* pHit1, HitSensor* pHit2) {
...
void MoguStone::emit(bool calcGravity, const TVec3f& vecA, const TVec3f& vecB, f32 speed) {
    mSpeed = speed;
    mTranslation.set(vecA);
    mVelocity.zero();
    MR::calcGravity(this);

    if (calcGravity) {
        MR::onCalcGravity(this);
        MR::vecKillElement(vecB, mGravity, &mFrontVec);
        mCalcGravity = true;
    }
    else {
